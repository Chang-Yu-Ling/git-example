...
let apiTemplate={
    ...
    'compare_get_header':{
                'nonbiz':'compare_get_header',
                'biz':'biz_compare_get_header',
                'param':'?prodId=prodIdReplace',
                'body':{
                }
    },
...
class Api{    
    public  get=(hostName:string,apiName:string,para:string):TE.TaskEither<Error,string>=>
        pipe(
            TE.tryCatch(
                ()=>axios.get(hostName+apiName+para,{ 'headers': { 'ca': '12345678' }}).then((response)=>{
...
const apiList=['compare_get','compare_post','compare_request','compare_get_header']  //test api list
